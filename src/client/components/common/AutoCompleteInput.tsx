...
const AutoCompleteInput = ({
  value, name,
  showSuggestion, handleAutoComplete,
  handleCloseSuggestion }: Props) => {
    ...
    const abc = airports
      ...
      .map((obj: airportObj, ind: number) => {
        ...
        return (<span onClick={() => {
          ...
          className="list__item" key={ind}>{title}</span>)
      }
      )
    return abc
  ...
  return (
    <>
      ...
      {showSuggestion && !isLoading && (
        ((value.length > 1 && airports != undefined) && (
          renderItem(name, value).length > 0 ?
            <ClickAwayListener onClickAway={handleCloseSuggestion} >
              <div className="input__suggestions__container">
                <div className="input__suggestions__content">
                  {renderItem(name, value)}
                </div>
              </div>
            </ClickAwayListener> : <Typography> No Record</Typography>))
      )}</>
  )
}
